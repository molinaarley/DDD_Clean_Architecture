...
import { View, Text, StyleSheet, TextInput, TouchableOpacity, Alert, Modal, ScrollView, ActivityIndicator, Dimensions } from 'react-native';
...
export function PhoneNumberScreen() {
  ...
  const { country, loading: locationLoading, error: locationError } = useLocation();
  const { set } = useFlowStore();
  ...
  const handleNext = () => {
    ...
    // Stocker le numéro dans le store
    set('phoneE164', phoneE164);
    console.log('Phone number:', phoneE164);
    (navigation as any).navigate('OtpVerification', { phoneE164 });
  };
        ...
        {locationError && !locationLoading && (
          <View style={styles.errorContainer}>
            <Text style={styles.errorText}>
              {locationError.includes('VPN') 
                ? 'Détection automatique indisponible (VPN détecté). Sélectionnez manuellement votre pays.'
                : 'Détection automatique indisponible. Sélectionnez manuellement votre pays.'
              }
            </Text>
          </View>
        )}
